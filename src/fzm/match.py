...
def match(s: str, data: list[str]) -> list[tuple]:
    """Return matches for some string."""
    # return process.extract(s, NAMES, scorer=fuzz.ratio)
    return [
        (x[0], str(x[1]), str(x[2])) for x in
        process.extract(s, data, scorer=fuzz.partial_ratio)
    ]


def match_iter(s: str, data: list[str]) -> Iterator[tuple[str, str, str]]:
    for result in process.extract_iter(s, data, scorer=fuzz.ratio):
